...
export function encodeHref(m: Model, href: string): string {
  ...
  encodeFlag(enc, VERSION_TABLE, m.version === 1 ? 0 : 1);
  if (m.version !== 1) encodeUniform(enc, m.version, 256);
...
export function codeVersion(code: string): number {
  ...
  const v = decodeFlag(dec, VERSION_TABLE);
  return v === 0 ? 1 : dec.uniform(256);
}
...
export function decodeCode(models: Map<number, Model>, code: string): string {
  const dec = new RangeDecoder(textToBytes(code));
  const v = decodeFlag(dec, VERSION_TABLE) === 0 ? 1 : dec.uniform(256);
...
export function explainHref(m: Model, href: string): { bits: number; parts: ExplainPart[] } {
  ...
  const versionBits = flagCost(VERSION_TABLE, m.version === 1 ? 0 : 1) / COST_SCALE;
  if (!s || !m.text.canCode(s.rest) || !m.host.canCode(s.host)) {
...
export function estimateBits(m: Model, href: string): number {
  ...
  const s = structure(url);
  if (!s || !m.text.canCode(s.rest) || !m.host.canCode(s.host)) {
    return (flagCost(m.flags.mode, 1) + m.text.parse(href, START_RAW).cost) / COST_SCALE;
  }
  const plan = planHost(m, s.host);
  let c = flagCost(m.flags.mode, 0) + flagCost(m.flags.scheme, s.https ? 0 : 1) + flagCost(m.flags.www, s.www ? 1 : 0);
  c += plan.cost;
